    ...
    public class RTS_Camera : MonoBehaviour {
        ...
        private void CameraUpdate() {
            ...
            if (Input.GetAxis("Mouse ScrollWheel") != 0) {
                ...
                Vector3 direction = Vector3.Normalize(desiredPosition - transform.position) * (distance * axis);

                if (transform.position.y >= maxHeight && axis < 0) {
                    return;
                }
                if (transform.position.y <= minHeight && axis > 0) {
                    return;
                }
